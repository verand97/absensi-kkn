    ...
    <div className="min-h-screen bg-[#0B0D14] text-white font-sans overflow-x-hidden relative selection:bg-[#7F56FF]/30 pb-24 md:pb-8">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 relative z-10">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6 p-px bg-slate-700/50" style={{ clipPath: "polygon(20px 0, 100% 0, 100% calc(100% - 20px), calc(100% - 20px) 100%, 0 100%, 0 20px)" }}>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-[#12141C] p-6 md:p-8 w-full" style={{ clipPath: "polygon(19px 0, 100% 0, 100% calc(100% - 19px), calc(100% - 19px) 100%, 0 100%, 0 19px)" }}>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
              <Link 
                href="/dashboard" 
                className="hidden md:flex items-center justify-center gap-2 bg-[#1A1C23] hover:bg-[#252836] border border-slate-700 text-slate-300 px-5 py-3 text-xs font-bold tracking-widest uppercase transition-colors"
              ...
              <Link 
                href="/scan" 
                className="hidden md:flex items-center justify-center gap-2 bg-linear-to-r from-purple-600 to-[#7F56FF] text-white px-5 py-3 text-xs font-bold tracking-widest uppercase transition-all hover:scale-[1.02] active:scale-[0.98] shadow-[0_0_15px_rgba(127,86,255,0.3)]"
                style={{ clipPath: "polygon(8px 0, 100% 0, 100% calc(100% - 8px), calc(100% - 8px) 100%, 0 100%, 0 8px)" }}
              >
        ...
        <div className="space-y-10">
          ...
          {/* Table Section */}
          <div className={`${activeTab === 'anggota' ? 'block' : 'hidden'} md:block p-px bg-slate-700/50`} style={{ clipPath: "polygon(20px 0, 100% 0, 100% calc(100% - 20px), calc(100% - 20px) 100%, 0 100%, 0 20px)" }}>
      ...
      <div className="fixed bottom-0 left-0 w-full bg-[#090A0F]/95 backdrop-blur-xl border-t border-slate-800/80 flex justify-between items-center px-6 py-3 md:hidden z-50">
        <Link href="/dashboard" className="flex flex-col items-center gap-1 w-16 opacity-40 hover:opacity-100 transition-opacity">
          <LayoutDashboard className="w-6 h-6 text-slate-300" strokeWidth={1.5} />
          <span className="text-[10px] font-medium text-slate-300">Member</span>
        </Link>
        <button onClick={() => setActiveTab('sesi')} className={`flex flex-col items-center gap-1 w-16 relative ${activeTab === 'sesi' ? 'opacity-100' : 'opacity-40 hover:opacity-100 transition-opacity'}`}>
          ...
          <Settings className={`w-6 h-6 ${activeTab === 'sesi' ? 'text-[#7F56FF]' : 'text-slate-300'}`} strokeWidth={activeTab === 'sesi' ? 2 : 1.5} />
          <span className={`text-[10px] font-medium ${activeTab === 'sesi' ? 'text-[#7F56FF]' : 'text-slate-300'}`}>Sesi</span>
        </button>
        <button onClick={() => setActiveTab('anggota')} className={`flex flex-col items-center gap-1 w-16 relative ${activeTab === 'anggota' ? 'opacity-100' : 'opacity-40 hover:opacity-100 transition-opacity'}`}>
          ...
          <Users className={`w-6 h-6 ${activeTab === 'anggota' ? 'text-[#80FF56]' : 'text-slate-300'}`} strokeWidth={activeTab === 'anggota' ? 2 : 1.5} />
          <span className={`text-[10px] font-medium ${activeTab === 'anggota' ? 'text-[#80FF56]' : 'text-slate-300'}`}>Anggota</span>
        </button>
        <Link href="/scan" className="flex flex-col items-center gap-1 w-16 opacity-40 hover:opacity-100 transition-opacity">
          <QrCode className="w-6 h-6 text-slate-300" strokeWidth={1.5} />
          <span className="text-[10px] font-medium text-slate-300">Scanner</span>
        </Link>
      </div>
